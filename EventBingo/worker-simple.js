...
export default {
  ...
  if (request.method === "GET" && path === "/event-info") {
    ...
    try {
      const eventData = await EventBingoProgress.get(`event_${eventCode}`);
    ...
    try {
      ...
      if (eventData) {
        ...
        if (!event.isLocked) {
          ...
          await EventBingoProgress.put(`event_${eventCode}`, JSON.stringify(event));
        }
      }
      ...
      await EventBingoPhotos.put(key, file);

      const progressKey = `${eventCode}_${player}`;
      let progress = await EventBingoProgress.get(progressKey);
      ...
      await EventBingoProgress.put(progressKey, JSON.stringify(progress));
  ...
  if (request.method === "GET" && path === "/progress") {
    ...
    const progressKey = `${eventCode}_${player}`;
    const progress = await EventBingoProgress.get(progressKey);
  ...
  if (request.method === "GET" && path === "/players") {
    const eventCode = getEventCode(request);
    const list = await EventBingoProgress.list({ prefix: `${eventCode}_` });
    const players = [];
  ...
  if (request.method === "GET" && path.startsWith("/photo/")) {
    ...
    const photo = await EventBingoPhotos.get(key);
  ...
  if (request.method === "GET" && path === "/photos") {
    const eventCode = getEventCode(request);
    const list = await EventBingoPhotos.list();
    const photos = [];
